# 00 — Preflight: Create Tasks Database

> Written 2026-08-05 by the orchestrator, reusing the toolchain facts verified end-to-end this session on the same repo/module (Podman backend, both test tiers green — see `../create-projects-database/00-preflight.md`). Deterministic for this repo.
...
| Language level | Java 21 (`<java.version>21</java.version>`); local JDK OpenJDK 23.0.2 |
...
./mvnw verify                                  # full tier incl. Testcontainers ITs (Podman env above must be exported)
./mvnw test -Dtest=CreateTasksDatabaseServiceTest#methodName
```
